	...
	program
		...
		.action((file: string, id: string, options: ShowOptions) => {
			try {
				...
				const individual = tree.indi(id as any);
				...
				if (options.json || options.format === "json") {
					...
					console.log(
						`# ${formatId(individual.id as any)} ${name}\n`
					);
					...
					if (father || mother) {
						...
						if (father) {
							...
							console.log(
								`- **Father:** ${father.id} ${fatherName} ${fatherLifespan}`
							);
						}
						if (mother) {
							...
							console.log(
								`- **Mother:** ${mother.id} ${motherName} ${motherLifespan}`
							);
						}
						console.log();
					}
					...
					if (spouses.length > 0) {
						...
						spouses.forEach((spouse) => {
							...
							console.log(
								`- ${spouse.id} ${spouseName} ${spouseLifespan}`
							);
						});
						console.log();
					}
					...
					if (children.length > 0) {
						...
						children.forEach((child) => {
							...
							console.log(
								`- ${child.id} ${childName} ${childLifespan}`
							);
						});
						console.log();
					}
					...
					console.log(
						formatHeader(
							`${formatId(individual.id as any)} ${formatName(name)}\n`
						)
					);
					if (sex) {
						console.log(
							`${formatLabel("Sex")} ${formatValue(sex === "M" ? "Male" : sex === "F" ? "Female" : sex)}`
						);
					}

					if (birthDate || birthPlace) {
						console.log(
							`${formatLabel("Born")} ${formatDate(birthDate)}`
						);
						if (birthPlace) {
							console.log(
								`${formatLabel("Birth Place")} ${formatPlace(birthPlace)}`
							);
						}
					}
					...
					if (deathDate || deathPlace) {
						console.log(
							`${formatLabel("Died")} ${formatDate(deathDate)}`
						);
						if (deathPlace) {
							console.log(
								`${formatLabel("Death Place")} ${formatPlace(deathPlace)}`
							);
						}
					}
					...
					if (father || mother) {
						...
						if (father) {
							...
							console.log(
								formatListItem(
									`${formatLabel("Father")} ${formatId(father.id as any)} ${formatName(fatherName)} ${fatherLifespan}`
								)
							);
						}
						if (mother) {
							...
							console.log(
								formatListItem(
									`${formatLabel("Mother")} ${formatId(mother.id as any)} ${formatName(motherName)} ${motherLifespan}`
								)
							);
						}
					}
					...
					if (spouses.length > 0) {
						...
						spouses.forEach((spouse) => {
							...
							console.log(
								formatListItem(
									`${formatId(spouse.id as any)} ${formatName(spouseName)} ${spouseLifespan}`
								)
							);
						});
					}
					...
					if (children.length > 0) {
						...
						children.forEach((child) => {
							...
							console.log(
								formatListItem(
									`${formatId(child.id as any)} ${formatName(childName)} ${childLifespan}`
								)
							);
						});
					}
