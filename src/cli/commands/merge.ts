...
export function registerMergeCommand(program: Command): void {
	program
		...
		.option(
			"--dedupe",
			"Attempt to detect and merge duplicates (basic implementation)"
		)
					...
					if (individuals) {
						individuals.forEach((indi) => {
							...
							if (options.dedupe) {
								...
								const duplicate = allIndividuals.find(
									(existing) => {
										return (
											existing.NAME?.toValue() === name &&
											existing.BIRT?.DATE?.toValue() ===
												birthDate
										);
									}
								);
							...
							// Ensure unique ID
							if (id) {
								while (seenIds.has(id as any)) {
									id = `@I${idCounter++}@` as any;
								}
								seenIds.add(id as any);
							}
